...
class PDF_1099_INT_ImageExtractor:
    """
...
#             plt.show()
    ...
    def process_single_pdf_1099_int(self):
                        ...
                        if process_1 and process_2:
                            ...
                            if output is not None:
                                if final_json.get(sentence_key) is None or final_json.get(sentence_key) == "":
                                    ...
                                    elif sentence_key in ["15 State","16 State identification no.","17 State tax withheld"]:
...
def handle_multiple_val_fields(sentence_key, extracted_sent):
    sentence=extracted_sent
    if sentence_key=="17 State tax withheld":
        pattern = r"17 State tax withheld\n([$] [0-9,.]+)\n([$] [0-9,.]+)\n"
        if re.search(pattern, sentence):
            tax_values = re.search(pattern, sentence).groups()
        else:
            tax_values = ['','']
        key1 = '17(1) State tax withheld'
        key2 = '17(2) State tax withheld'
        value1 = tax_values[0]
        value2 = tax_values[1]
    elif sentence_key=="15 State":
        pattern = r"15 State\n.*([A-Z]{2}).*\n.*([A-Z]{2}).*\n"
        if re.search(pattern, sentence):
            state_values = re.search(pattern, sentence).groups()
        else:
            state_values = ['','']
        key1 = '15(1) State'
        key2 = '15(2) State'
        value1 = state_values[0]
        value2 = state_values[1]
    else:
        sentence.replace('_','')
        pattern = r"16 State identification no.\n(\d+)\n(\d+)\n"
        if re.search(pattern, sentence):
            stateid_values = re.search(pattern, sentence).groups()
        else:
            stateid_values = ['','']
        key1 = '16(1) State identification no.'
        key2 = '16(2) State identification no.'
        value1 = stateid_values[0]
        value2 = stateid_values[1]
