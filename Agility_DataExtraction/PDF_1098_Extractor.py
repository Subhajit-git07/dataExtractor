...
class PDF_1098_ImageExtractor:
    """
    centriod_closeness_threshold=90 #For 1098
    threshold=0.9 #For 1098
    page_number=1 (Page Number to convert into image)
    """
    def __init__(self, pdf_file_path, output_image_path, template_json, threshold=0.9,page_number=1,centriod_closeness_threshold=90, threshold_cosine = 0.70):
    ...
    def process_single_pdf_1098(self):
            ...
            for d in cleaned_array_processed:
                for key, value in d.items():
                    ...
                    if len(sentence_extract) >= 1:
                        if sentence_extract[0] == "‘":
                            sentence_extract = sentence_extract[1:]
                        if sentence_key == "8 Address or description of property securing mortgage":
                            sentence_extract = sentence_extract.replace("see", "").replace("instructions", "")
                            sentence_extract = sentence_extract.replace("Address of", "Address or")
                            special_chars = "{}[]()"
                            sentence_extract = ''.join(char for char in sentence_extract if char not in special_chars)
                        ...
                        process_2 = func_check_similarity(
                            ...
                            sentence_extract[:math.floor(len(sentence_key) / 3)], self.threshold_cosine)
                        ...
                        if process_1 and process_2:
                            ...
                            if output is not None:
                                if final_json.get(sentence_key) is None or final_json.get(sentence_key) == "":
                                    ...
                                    # interim_dict["Confidence"] = "100"
                                    # interim_dict["Span"] = {"Offset": 0, "Length": len(output)}
                                    final_json[sentence_key] = interim_dict

        return final_json


def func_check_similarity(sentence1, sentence2, threshold_value = 0.70):
    ...
    #     print("sentence1",sentence1, cosine_sim)
